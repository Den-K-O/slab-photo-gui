...
def get_points(img,n):
    ...
    def click_corners(event, x, y, flags, param):    
        ...
        if event == cv2.EVENT_LBUTTONUP:
            points.append((x, y))
            cv2.rectangle(bgimage, (x-5,y-5), (x+5,y+5), (0, 255, 0), 2)
    ...
    while True:
        ...
        image_copy = bgimage.copy()
        mx,my = mouse_raw
        #print (mouse_raw)
        cv2.line(image_copy,(int(mx),0),(int(mx),int(height)),(0, 255, 0),2)
        cv2.line(image_copy,(0,int(my)),(int(width),int(my)),(0, 255, 0),2)        
